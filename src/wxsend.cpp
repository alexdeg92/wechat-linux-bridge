...
// ── Configuration ─────────────────────────────────────────────────────────────
// DLL listens on this port for send commands from the Python host
#define WXSEND_LISTEN_PORT 19088

// Outbound webhook: the Python receiver listens here for incoming messages
// Change to match your WEBHOOK_PORT env variable
#define WEBHOOK_HOST "127.0.0.1"
#define WEBHOOK_PORT 19089
#define WEBHOOK_PATH "/webhook/message"

// Shared file path (Z: maps to Linux root in Wine)
// Must match INCOMING_FILE in wechat_webhook_receiver.py
#define INCOMING_JSONL_PATH "Z:\\tmp\\wechat_incoming.jsonl"
// ─────────────────────────────────────────────────────────────────────────────
...
static DWORD WINAPI WebhookPostThread(LPVOID param) {
    ...
    addr.sin_port = htons(WEBHOOK_PORT);
    addr.sin_addr.s_addr = inet_addr(WEBHOOK_HOST);
    ...
    sprintf(request,
        ...
        "\r\n"
        "%s",
        WEBHOOK_PORT, jsonLen, payload->json);
...
static DWORD WINAPI HttpThread(LPVOID) {
    ...
    struct sockaddr_in addr = {}; addr.sin_family = AF_INET; addr.sin_port = htons(WXSEND_LISTEN_PORT); addr.sin_addr.s_addr = inet_addr("127.0.0.1");
